...
You can use the Bitbucket Pipeline system to run Garda Code Action on every pull request open or update.
...
pipelines:
    pull-requests:
      '**':
        - step:
            name: Garda Code Action Review
            image: neosantara/garda-code-action:latest
            script:
              - garda-code-action --pr_url=https://bitbucket.org/$BITBUCKET_WORKSPACE/$BITBUCKET_REPO_SLUG/pull-requests/$BITBUCKET_PR_ID review
```
...
To run Garda Code Action as webhook, build the docker image:

```bash
docker build . -t neosantara/garda-code-action:bitbucket_server_webhook --target bitbucket_server_webhook -f docker/Dockerfile
docker push neosantara/garda-code-action:bitbucket_server_webhook  # Push to your Docker repository
```

...
You can use a pre-built Action Docker image to run Garda Code Action as an Azure DevOps pipeline.
...
stages:
- stage: garda
  displayName: 'Garda Code Action Stage'
  jobs:
  - job: garda_job
    displayName: 'Garda Code Action Job'
    pool:
      vmImage: 'ubuntu-latest'
    container:
      image: neosantara/garda-code-action:latest
      options: --entrypoint ""
    variables:
      - group: garda
    steps:
    - script: |
        ...
        garda-code-action --pr_url="$PR_URL" describe
        garda-code-action --pr_url="$PR_URL" review
        garda-code-action --pr_url="$PR_URL" improve
      env:
        azure_devops__pat: $(azure_devops_pat)
        openai__key: $(OPENAI_KEY)
      displayName: 'Run Garda Code Action'
```

This script will run Garda Code Action on every new merge request, with the `improve`, `review`, and `describe` commands.
...
Make sure to give pipeline permissions to the `garda` variable group.

> Note that Azure Pipelines lacks support for triggering workflows from PR comments. If you find a viable solution, please contribute it to our [issue tracker](https://github.com/neosantara-xyz/garda-code-action/issues)

### Azure Repos Git PR triggers and Build Validation

Azure Repos Git does not use YAML `pr:` triggers for pipelines. Instead, configure Build Validation on the target branch to run the Garda Code Action pipeline for pull requests:
...
3. Under Build Validation, add a policy:
   - Select the Garda Code Action pipeline (the `azure-pipelines.yml` above).
   - Set it as Required.

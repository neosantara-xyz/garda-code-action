To run Garda Code Action locally, you first need to acquire two keys:
...
To invoke a tool (for example `review`), you can run Garda Code Action directly from the Docker image. Here's how:

- For GitHub:

    ```bash
    docker run --rm -it -e OPENAI.KEY=<your_openai_key> -e GITHUB.USER_TOKEN=<your_github_token> neosantara/garda-code-action:latest --pr_url <pr_url> review
    ```
...
- For GitLab:
    ...
    ```bash
    docker run --rm -it -e OPENAI.KEY=<your key> -e CONFIG.GIT_PROVIDER=gitlab -e GITLAB.PERSONAL_ACCESS_TOKEN=<your token> neosantara/garda-code-action:latest --pr_url <pr_url> review
    ```
...
- For BitBucket:
    ...
    ```bash
    docker run --rm -it -e CONFIG.GIT_PROVIDER=bitbucket -e OPENAI.KEY=$OPENAI_API_KEY -e BITBUCKET.BEARER_TOKEN=$BITBUCKET_BEARER_TOKEN neosantara/garda-code-action:latest --pr_url=<pr_url> review
    ```

- For Gitea:

    ```bash
    docker run --rm -it -e OPENAI.KEY=<your key> -e CONFIG.GIT_PROVIDER=gitea -e GITEA.PERSONAL_ACCESS_TOKEN=<your token> neosantara/garda-code-action:latest --pr_url <pr_url> review
    ```
...
For other git providers, update `CONFIG.GIT_PROVIDER` accordingly and check the [`garda/settings/.secrets_template.toml`](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/.secrets_template.toml) file for environment variables expected names and values.
...
For example, suppose you want to run `garda` that connects to a self-hosted GitLab instance similar to an example above.
...
Then, you can run `garda` using Docker with the following command:

```shell
docker run --rm -it --env-file .env neosantara/garda-code-action:latest <tool> <tool parameter>
```
...
Note that litellm, which is used by garda-code-action, sometimes returns non-informative error messages such as `APIError: OpenAIException - Connection error.`
...
def main():
    ...
    pr_url = "..."      # PR URL, for example 'https://github.com/neosantara-xyz/garda-code-action/pull/809'
...
2. Navigate to the `/garda-code-action` folder and install the requirements in your favorite virtual environment:
...
chmod 600 garda/settings/.secrets.toml
...
```bash
python3 -m garda.cli --pr_url <pr_url> review
python3 -m garda.cli --pr_url <pr_url> ask <your question>
python3 -m garda.cli --pr_url <pr_url> describe
python3 -m garda.cli --pr_url <pr_url> improve
python3 -m garda.cli --pr_url <pr_url> add_docs
python3 -m garda.cli --pr_url <pr_url> generate_labels
python3 -m garda.cli --issue_url <issue_url> similar_issue
...
```

[Optional] Add the garda folder to your PYTHONPATH

```bash
export PYTHONPATH=$PYTHONPATH:<PATH to garda folder>
```

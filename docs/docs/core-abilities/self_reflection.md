...
Garda Code Action implements a **self-reflection** process where the AI model reflects, scores, and re-ranks its own suggestions, eliminating irrelevant or incorrect ones.
...
To achieve this goal, Garda Code Action offers a dedicated hierarchical structure when presenting suggestions to users:
...
To conclude, the self-reflection process enables Garda Code Action to prioritize suggestions based on their importance, eliminate inaccurate or irrelevant proposals, and optionally exclude suggestions that fall below a specified threshold of significance.

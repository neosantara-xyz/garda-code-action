...
Garda Code Action streamlines code review workflows by seamlessly connecting with multiple ticket management systems.
...
Garda Code Action will recognize the ticket and use the ticket content (title, description, labels) to provide additional context for the code changes.
...
Garda Code Action will automatically recognize GitHub/Gitlab issues mentioned in the PR description and fetch the issue content.
...
Since Garda Code Action is integrated with GitHub, it doesn't require any additional configuration to fetch GitHub issues.
...
If you are facing issues retrieving tickets in Garda Code Action with Basic auth, you can validate the flow using a Python script.
...
If you are facing issues retrieving tickets in Garda Code Action with PAT token, you can validate the flow using a Python script.
...
Garda Code Action supports connecting to multiple JIRA servers using different authentication methods.

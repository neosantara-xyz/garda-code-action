...
By default, Garda Code Action tries to preserve your original PR description by placing it above the generated content.
This requires including your description during the initial PR creation.

"Garda Code Action removed the original description from the PR. Why"?
...
???+ example "Possible configurations"
    ...
    <table>
      ...
      <tr>
        ...
        <td>If set to true, it will add a comment message [`PR Description updated to latest commit...`](https://github.com/neosantara-xyz/garda-code-action/pull/499#issuecomment-1837412176) after finishing calling `/describe`. Default is true.</td>
      </tr>
      <tr>
...
The marker `garda:type` will be replaced with the PR type, `garda:summary` will be replaced with the PR summary, `garda:walkthrough` will be replaced with the PR walkthrough, and `garda:diagram` will be replaced with the sequence diagram (if enabled).
...
- `use_description_markers`: if set to true, the tool will use markers template. It replaces every marker of the form `garda:marker_name` with the relevant content. Default is false.
- `include_generated_by_header`: if set to true, the tool will add a dedicated header: 'Generated by Garda Code Action at ...' to any automatic content. Default is true.
...
- Description: Start the description of with prefix `garda:`, for example: `garda: Description of when AI should suggest this label`.<br>
...
!!! tip "Automation"
    - When you first install Garda Code Action app, the [default mode](../usage-guide/automations_and_usage.md#github-app) for the describe tool is:
    ```

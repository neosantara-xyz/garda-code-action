...
(Read more about the different personas in the PR process and how Garda Code Action aims to assist them in our [blog](https://neosantara.xyz/blog/understanding-the-challenges-and-pain-points-of-the-pull-request-cycle/))
...
!!! tip ""
    When you first install Garda Code Action app, the [default mode](../usage-guide/automations_and_usage.md#github-app-automatic-tools-when-a-new-pr-is-opened) for the `review` tool is:
    ```
    pr_commands = ["/review", ...]
    ```
...
!!! tip ""
    ...
    - **`possible security issue`**: This label is applied if the tool detects a potential [security vulnerability](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/pr_reviewer_prompts.toml#L134) in the PR's code. This feedback is controlled by the 'enable_review_labels_security' flag (default is true).
    - **`review effort [x/5]`**: This label estimates the [effort](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/pr_reviewer_prompts.toml#L118) required to review the PR on a relative scale of 1 to 5, where 'x' represents the assessed effort. This feedback is controlled by the 'enable_review_labels_effort' flag (default is true).

...
Garda Code Action supports two modes for presenting code suggestions: 
...
Table mode is the default of Garda Code Action, and is recommended approach for most users due to these benefits. 
...
Garda Code Action supports both simple and hierarchical best practices configurations to provide guidance to the AI model for generating relevant code suggestions.
...
Garda Code Action uses a dynamic strategy to generate code suggestions based on the size of the pull request (PR). Here's how it works:

#### 1. Chunking large PRs

- Garda Code Action divides large PRs into 'chunks'.
- Each chunk contains up to `config.max_model_tokens` tokens (default: 32,000).

#### 2. Generating suggestions

- For each chunk, Garda Code Action generates up to `pr_code_suggestions.num_code_suggestions_per_chunk` suggestions (default: 3).
...
Note: Chunking is primarily relevant for large PRs. For most PRs (up to 600 lines of code), Garda Code Action will be able to process the entire code in a single call.

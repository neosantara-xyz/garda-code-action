...
The possible configurations of Garda Code Action are stored in [here](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml){:target="_blank"}.
...
In some cases, you may want to exclude specific files or directories from the analysis performed by Garda Code Action. This can be useful, for example, when you have files that are generated automatically or files that shouldn't be reviewed, like vendor code.
...
All Garda Code Action tools have a parameter called `extra_instructions`, that enables to add free-text extra instructions. Example usage:
...
The default response language for Garda Code Action is **U.S. English**. However, some development teams may prefer to display information in a different language. For example, your team's workflow might improve if PR descriptions and code suggestions are set to your country's native language.
...
[//]: # ()
[//]: # (The default mode of Neosantara is to have a single call per tool, using GPT-4, which has a token limit of 8000 tokens.)
...
By default, GitLab merge requests show submodule updates as `Subproject commit` lines. To include the actual file-level changes from those submodules in Garda Code Action analysis, enable:
...
When enabled, Garda Code Action will fetch and attach diffs from the submodule repositories. The default is `false` to avoid extra GitLab API calls.

## Log Level

Garda Code Action allows you to control the verbosity of logging by using the `log_level` configuration parameter. This is particularly useful for troubleshooting and debugging issues with your PR workflows.
...
Configure callbacks according to the LiteLLM documentation for the observability backend you use.

## Bringing additional repository metadata to Garda Code Action

To provide Garda Code Action tools with additional context about your project, you can enable automatic repository metadata detection. 
...
Garda Code Action automatically searches for repository metadata files in your PR's head branch root directory. By default, it looks for:
`AGENTS.md`, `GARDA.md`, and `CLAUDE.md`.
...
Garda Code Action allows you to automatically ignore certain PRs based on various criteria:
...
Garda Code Action tries to automatically identify and ignore pull requests created by bots using:
...
You can view the list of auto-generated file patterns in [`generated_code_ignore.toml`](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/generated_code_ignore.toml).
Files matching these glob patterns will be automatically excluded from Garda Code Action analysis.

### Ignoring Tickets with Specific Labels

When Garda Code Action analyzes tickets (JIRA, GitHub Issues, GitLab Issues, etc.) referenced in your PR, you may want to exclude tickets that have certain labels from the analysis. This is useful for filtering out tickets marked as "ignore-compliance", "skip-review", or other labels that indicate the ticket should not be considered during PR review.

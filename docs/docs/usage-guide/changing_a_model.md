## Changing a model in Garda Code Action

See [here](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/algo/__init__.py) for a list of supported models in Garda Code Action.
The default model of Garda Code Action is `GPT-5` from OpenAI.
To use a different model than the default, you need to edit in the [configuration file](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml#L7) the fields:
...
By default, Ollama uses a context window size of 2048 tokens. In most cases this is not enough to cover garda-code-action prompt and pull-request diff. Context window size can be overridden with the `OLLAMA_CONTEXT_LENGTH` environment variable. For example, to set the default context length to 8K, use: `OLLAMA_CONTEXT_LENGTH=8192 ollama serve`. More information you can find on the [official ollama faq](https://github.com/ollama/ollama/blob/main/docs/faq.md#how-can-i-specify-the-context-window-size).
...
!!! note "Local models vs commercial models"
    Garda Code Action is compatible with almost any AI model, but analyzing complex code repositories and pull requests requires a model specifically optimized for code analysis.
...
Also, review the [.secrets_template.toml](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/.secrets_template.toml) file for instructions on how to set keys for other models.
...
To use xAI's models with Garda Code Action, set:
...
If the relevant model doesn't appear [here](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/algo/__init__.py), you can still use it as a custom model:

...
When running from your locally cloned Garda Code Action repo (CLI), your local configuration file will be used.
Examples of invoking the different tools via the CLI:

- **Review**:       `python -m garda.cli --pr_url=<pr_url>  review`
- **Describe**:     `python -m garda.cli --pr_url=<pr_url>  describe`
- **Improve**:      `python -m garda.cli --pr_url=<pr_url>  improve`
- **Ask**:          `python -m garda.cli --pr_url=<pr_url>  ask "Write me a poem about this PR"`
- **Update Changelog**:      `python -m garda.cli --pr_url=<pr_url>  update_changelog`

`<pr_url>` is the url of the relevant PR (for example: [#50](https://github.com/neosantara-xyz/garda-code-action/pull/50)).
...
```
python -m garda.cli --pr_url=<pr_url>  /review --pr_reviewer.extra_instructions="focus on the file: ..."
```
...
3. **git provider**: The [git_provider](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml#L12) field in a configuration file determines the GIT provider that will be used by Garda Code Action. Currently, the following providers are supported:
`github` **(default)**, `gitlab`, `bitbucket`, `azure`, `local`, and `gitea`.

### CLI Health Check

To verify that Garda Code Action has been configured correctly, you can run this health check command from the repository root:
...
Online usage means invoking Garda Code Action tools by [comments](https://github.com/neosantara-xyz/garda-code-action/pull/229#issuecomment-1695021901) on a PR.
...
Any configuration value in [configuration file](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml) file can be similarly edited. Comment `/config` to see the list of available configurations.

## Garda Code Action Automatic Feedback

### Disabling all automatic feedback

To easily disable all automatic feedback from Garda Code Action (GitHub App, GitLab Webhook, BitBucket App, Azure DevOps Webhook), set in a configuration file:
...
When this parameter is set to `true`, Garda Code Action will not run any automatic tools (like `describe`, `review`, `improve`) when a new PR is opened, or when new code is pushed to an open PR.

### GitHub App

!!! note "Configurations for Garda Code Action"
    Garda Code Action for GitHub is an App, hosted by Neosantara. So all the instructions below are relevant for Garda Code Action users.
    Same goes for [GitLab webhook](#gitlab-webhook) and [BitBucket App](#bitbucket-app) sections.

#### GitHub app automatic tools when a new PR is opened

The [github_app](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml#L223) section defines GitHub app specific configurations.
...
This means that when a new PR is opened/reopened or marked as ready for review, Garda Code Action will run the `describe`, `review` and `improve` tools.  
...
This means that when new code is pushed to the PR, Garda Code Action will run the `describe` and `review` tools, with the specified parameters.

### GitHub Action

`GitHub Action` is a different way to trigger Garda Code Action tools, and uses a different configuration mechanism than `GitHub App`.<br>
You can configure settings for `GitHub Action` by adding environment variables under the env section in `.github/workflows/garda.yml` file.
...
The JSON structure is equivalent to the yaml data structure defined in [pr_reviewer_prompts.toml](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/pr_reviewer_prompts.toml).

Note that you can give additional config parameters by adding environment variables to `.github/workflows/garda.yml`, or by using a `.garda.toml` [configuration file](./configuration_options.md#global-configuration-file) in the root of your repo

For example, you can set an environment variable: `pr_description.publish_labels=false`, or add a `.garda.toml` file with the following content:
...
to prevent Garda Code Action from publishing labels when running the `describe` tool.
...
When this is configured, Garda Code Action can be invoked by commenting on the PR.
...
For complete model configuration details, see [Changing a model in Garda Code Action](changing_a_model.md).
...
Similar to GitHub app, when running Garda Code Action from BitBucket App, the default [configuration file](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml) will be initially loaded.

By uploading a local `.garda.toml` file to the root of the repo's default branch, you can edit and customize any configuration parameter. Note that you need to upload `.garda.toml` prior to creating a PR, in order for the configuration to take effect.

For example, if your local `.garda.toml` file contains:
...
If you experience a lack of responses from Garda Code Action, you might want to set: `bitbucket_app.avoid_full_files=true` in your configuration file.
This will prevent Garda Code Action from acquiring the full file content, and will only use the diff content. This will reduce the number of requests made to BitBucket, at the cost of small decrease in accuracy, as dynamic context will not be applicable.

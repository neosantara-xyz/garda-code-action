...
from garda.algo.types import EDIT_TYPE
from garda.config_loader import get_settings, global_settings
from garda.mosaico.diff_provider import DiffInputProvider, parse_unified_diff
...
class TestProviderRegistration:
    def test_registry_has_mosaico_diff_and_originals_intact(self):
        import garda.mosaico.provider_registration  # noqa: F401 (triggers setdefault)
...
def _setup_request_settings(verb_provider_input):
    ...
    s.set("CONFIG.AI_HANDLER", "litellm")
...
def isolated_settings():
    ...
    keys = ["CONFIG.GIT_PROVIDER", "CONFIG.PUBLISH_OUTPUT", "CONFIG.AI_HANDLER"]
    sentinel = object()
...
async def _run_tool_via_spy(monkeypatch, isolated_settings, verb, canned_yaml, second_yaml=""):
    ...
    from garda.git_providers import _GIT_PROVIDERS
    import garda.algo.ai_handlers.litellm_ai_handler as litellm_mod
    ...
    ok = await GardaAgent().handle_request("mosaico://supplied-diff", ["/" + verb])

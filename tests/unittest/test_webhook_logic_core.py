...
import garda.servers.bitbucket_server_webhook as bitbucket_server_webhook
...
def gitlab_webhook_module():
    ...
    try:
        module = importlib.import_module("garda.servers.gitlab_webhook")
        yield module
    finally:
...
def test_gitlab_should_process_pr_logic_ignores_labels_and_branches(gitlab_webhook_module):
    ...
    settings.set("CONFIG.IGNORE_PR_LABELS", ["skip-garda-code-action"])
    settings.set("CONFIG.IGNORE_PR_SOURCE_BRANCHES", ["^generated/"])
    settings.set("CONFIG.IGNORE_PR_TARGET_BRANCHES", ["^legacy$"])

    try:
        assert gitlab_webhook_module.should_process_pr_logic(
            _gitlab_payload(labels=[{"title": "skip-garda-code-action"}])
        ) is False
        assert gitlab_webhook_module.should_process_pr_logic(

from garda.git_providers.utils import handle_configurations_errors
...
def test_handle_configurations_errors_uses_persistent_comment_when_supported():
    ...
    assert comment["initial_header"] == "❌ **Garda Code Action failed to apply 'local' repo settings**"
    assert comment["update_header"] is False
    assert comment["final_update_message"] is False
    assert "Garda Code Action failed to apply 'local' repo settings" in comment["body"]
...
def test_handle_configurations_errors_keeps_markdown_details_when_persistent_comment_is_missing():
    ...
    assert "Garda Code Action failed to apply 'local' repo settings" in provider.comments[0]

...
Note: import garda.config_loader first to avoid the garda.log <->
custom_merge_loader circular import (mirrors server.py)."""
import os

import garda.config_loader  # noqa: F401  (import-order load; see module docstring)
...
class TestA2ARoundTripStubbedLLM:
    ...
    async def test_warmup_health_and_card(self, monkeypatch):
        ...
        from garda.mosaico.server import build_app
        ...
        async with _build_client(app) as client:
            ...
            body = card.json()
            assert body["name"] == "Garda Code Action Solution Agent"
    ...
    async def test_supplied_diff_review_roundtrip(self, monkeypatch):
        ...
        assert "Garda Review Guide" in text

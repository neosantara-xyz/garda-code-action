"""Tests for GardaAgentExecutor.execute.
...
import garda.mosaico.executor as executor_mod
from garda.mosaico.executor import GardaAgentExecutor
...
class TestExecute:
    ...
    async def test_completes_with_rendered_markdown(self, monkeypatch, spy_updater):
        ...
        with request_cycle_context({}):
            await GardaAgentExecutor().execute(ctx, eq)
    ...
    async def test_empty_render_uses_placeholder(self, monkeypatch, spy_updater):
        ...
        monkeypatch.setattr(executor_mod, "route_and_run", fake_route_and_run)
        with request_cycle_context({}):
            await GardaAgentExecutor().execute(_FakeRequestContext("x"), _RecordingEventQueue())
    ...
    async def test_route_and_run_raises_marks_failed(self, monkeypatch, spy_updater):
        ...
        with request_cycle_context({}):
            await GardaAgentExecutor().execute(_FakeRequestContext("y"), eq)
    ...
    async def test_cancel_raises_not_implemented(self):
        with pytest.raises(NotImplementedError):
            await GardaAgentExecutor().cancel(_FakeRequestContext("z"), _RecordingEventQueue())

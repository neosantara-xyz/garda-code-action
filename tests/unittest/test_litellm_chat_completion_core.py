...
async def test_chat_completion_passes_seed_when_temperature_is_zero(monkeypatch):
    ...
    with patch("garda.algo.ai_handlers.litellm_ai_handler.acompletion", new_callable=AsyncMock) as mock_call:
...
async def test_get_completion_uses_streaming_for_required_models():
    ...
    with patch("garda.algo.ai_handlers.litellm_ai_handler.acompletion", new_callable=AsyncMock) as mock_call, \
            patch("garda.algo.ai_handlers.litellm_ai_handler._handle_streaming_response",

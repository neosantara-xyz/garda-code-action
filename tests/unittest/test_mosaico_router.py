...
from garda.config_loader import global_settings
from garda.mosaico.dispatch import (_detect_verb, _empty_fallback,
...
class TestPathPrUrl:
    ...
    async def test_pr_url_runs_handle_request_and_returns_artifact(self, monkeypatch, restore_settings):
        ...
        from garda.agent.garda_agent import GardaAgent
        monkeypatch.setattr(GardaAgent, "handle_request", fake_handle_request)

        out = await route_and_run(f"review {PR_URL}")
        assert out == "REVIEW MARKDOWN"
        assert captured["pr_url"] == PR_URL
        # _run_garda must inject the no-publish flags so tools write into
...
class TestPathSuppliedDiff:
    ...
    async def test_diff_with_question_mark_in_body_still_reviews(self, monkeypatch, restore_settings):
        ...
        from garda.agent.garda_agent import GardaAgent
        monkeypatch.setattr(GardaAgent, "handle_request", fake_handle_request)
        monkeypatch.setattr("garda.tools.pr_questions.PRQuestions", _explode_prquestions)
...
class TestAskWithContext:
    ...
    async def test_pr_url_question_runs_prquestions(self, monkeypatch, restore_settings):
        ...
        garda_used = {"called": False}

        async def fail_handle_request(self, *a, **k):
            garda_used["called"] = True
            return True

        monkeypatch.setattr("garda.tools.pr_questions.PRQuestions", FakePRQuestions)
        from garda.agent.garda_agent import GardaAgent
        monkeypatch.setattr(GardaAgent, "handle_request", fail_handle_request)
        ...
        assert garda_used["called"] is False
...
class TestPathFreeText:
    ...
    async def test_free_text_returns_guidance_not_internal_error(self, monkeypatch, restore_settings):
        """A context-free free-text ask must NOT call PRQuestions/GardaAgent and must NOT
        ...
        assert out == "Garda Code Action requires a PR URL or a supplied diff."
...
class TestDefensiveCapture:
    ...
    async def test_ask_that_raises_returns_error_fallback(self, monkeypatch, restore_settings):
        ...
        monkeypatch.setattr("garda.tools.pr_questions.PRQuestions", RaisingPRQuestions)
    ...
    async def test_route_and_run_never_raises_on_garbage(self, restore_settings):
        # No monkeypatching: a host-less Garda Code Action run / ask should still return a string.
...
class TestPublishOutputForced:
    """Prove that _run_garda and _run_ask force publish_output=False regardless
    ...
    @pytest.mark.asyncio
    async def test_run_garda_injects_no_publish_flags(self, monkeypatch, restore_settings):
        """_run_garda must pass --config.publish_output=false and
        ...
        from garda.agent.garda_agent import GardaAgent
        from garda.mosaico.dispatch import _run_garda
        monkeypatch.setattr(GardaAgent, "handle_request", fake_handle_request)
        ...
        out = await _run_garda(PR_URL, "review")
    ...
    async def test_run_ask_forces_publish_output_false(self, monkeypatch, restore_settings):
        ...
        arg-injection trick used by _run_garda cannot apply; a settings.set() is
        required instead."""
        ...
        monkeypatch.setattr("garda.tools.pr_questions.PRQuestions", CapturingPRQuestions)

        from garda.mosaico.dispatch import _run_ask

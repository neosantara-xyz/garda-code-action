...
import garda.agent.garda_agent as garda_module
...
def _patch_request_dependencies(monkeypatch, validate_result=(True, None), update_settings_fn=None):
    ...
    monkeypatch.setattr(garda_module, "apply_repo_settings", lambda pr_url: None)
    monkeypatch.setattr(garda_module.CliArgs, "validate_user_args", lambda args: validate_result)
    monkeypatch.setattr(garda_module, "update_settings_from_args", update_settings_fn)
...
async def test_handle_request_routes_known_command_and_notifies(monkeypatch):
    ...
    monkeypatch.setitem(garda_module.command2class, "custom", FakeTool)

    handled = await garda_module.GardaAgent(ai_handler="fake-ai")._handle_request(
...
async def test_handle_request_rejects_forbidden_cli_args(monkeypatch):
    ...
    handled = await garda_module.GardaAgent()._handle_request("https://example/pr/1", "/custom --secret=value")
...
async def test_handle_request_wrapper_returns_false_on_exception(monkeypatch):
    ...
    monkeypatch.setattr(garda_module.GardaAgent, "_handle_request", raise_error)

    handled = await garda_module.GardaAgent().handle_request("https://example/pr/1", "/review")
...
async def test_handle_request_answer_uses_reviewer_answer_mode_and_notifies(monkeypatch):
    ...
    _patch_request_dependencies(monkeypatch)
    monkeypatch.setattr(garda_module, "PRReviewer", FakeReviewer)
...
async def test_handle_request_answer_preserves_quoted_question_as_single_arg(monkeypatch):
    ...
    handled = await garda_module.GardaAgent()._handle_request(
...
async def test_handle_request_returns_false_for_unknown_command(monkeypatch):
    ...
    handled = await garda_module.GardaAgent()._handle_request("https://example/pr/1", "/unknown")

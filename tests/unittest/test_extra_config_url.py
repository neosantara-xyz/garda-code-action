...
from garda.cli import set_parser
from garda.config_loader import get_settings
from garda.git_providers import utils as git_utils
from garda.git_providers.utils import (
...
class _CapturingHandler(BaseHTTPRequestHandler):
    ...
    body = SAMPLE_TOML
    expected_path = "/shared.garda.toml"
...
def http_server():
    ...
    _CapturingHandler.expected_path = "/shared.garda.toml"
...
def test_resolve_fetches_http_url_into_tempfile(http_server):
    url = f"{http_server}/shared.garda.toml"
    path, is_temp = _resolve_extra_config_to_file(url)
    try:
...
def test_resolve_injects_auth_header_from_env(http_server, monkeypatch):
    ...
    monkeypatch.setenv("GARDA_EXTRA_CONFIG_AUTH_HEADER", f"PRIVATE-TOKEN: {fake_token}")
...
def test_resolve_returns_none_when_auth_header_missing(http_server, monkeypatch):
    ...
    monkeypatch.delenv("GARDA_EXTRA_CONFIG_AUTH_HEADER", raising=False)
...
def test_resolve_malformed_auth_header_warns_and_drops(http_server, monkeypatch):
    ...
    _CapturingHandler.require_header = None
    monkeypatch.setenv("GARDA_EXTRA_CONFIG_AUTH_HEADER", "no-colon-here")
    ...
    try:
        ...
        assert is_temp is True
        combined = "\n".join(captured_lines)
        assert "GARDA_EXTRA_CONFIG_AUTH_HEADER" in combined and "malformed" in combined.lower(), (
...
def test_cli_parser_accepts_extra_config_url(monkeypatch):
    ...
    monkeypatch.delenv("GARDA_EXTRA_CONFIG_URL", raising=False)
    parser = set_parser()
...
def test_cli_parser_defaults_to_env_var_when_flag_omitted(monkeypatch):
    monkeypatch.setenv("GARDA_EXTRA_CONFIG_URL", "/tmp/shared.toml")
    parser = set_parser()
...
def test_cli_parser_flag_takes_precedence_over_env_var(monkeypatch):
    monkeypatch.setenv("GARDA_EXTRA_CONFIG_URL", "/from/env.toml")
    parser = set_parser()
    args = parser.parse_args([
        "--pr_url=https://example.com/pr/1",
...
def test_cli_setting_reconciles_between_runs(settings_sandbox, monkeypatch):
    ...
    import garda.cli as cli_mod

    # Stub GardaAgent so run() returns quickly without making network calls;
    # we only care about the synchronous setting-reconciliation prologue.
    class _StubAgent:
        async def handle_request(self, *_args, **_kwargs):
            return True

    monkeypatch.setattr(cli_mod, "GardaAgent", lambda: _StubAgent())
    monkeypatch.delenv("GARDA_EXTRA_CONFIG_URL", raising=False)
...
# configuration shipped by garda-code-action.
...
def test_apply_settings_file_does_not_log_secret_values(tmp_path, settings_sandbox):
    ...
    from the merged config, otherwise secrets in external .garda.toml
    ...
    garda-code-action uses loguru; pytest's capsys/caplog don't capture it because the
...
def test_extra_applied_when_repo_settings_empty(tmp_path, settings_sandbox, mock_git_provider):
    """If the repo has no .garda.toml, extra values still take effect."""
    extra_path = _write_toml(tmp_path, "extra.toml", f"""
[{_TEST_SECTION}]
...
""")
    ...
    assert get_settings().get(f"{_TEST_SECTION}.token") == "from-env", (
        ...
        "provider token from GARDA_<SECTION>__<KEY> can be silently "
...
def test_env_var_overrides_repo_settings(tmp_path, settings_sandbox, mock_git_provider, monkeypatch):
    """Same precedence rule applies to the repo-local .garda.toml merge:
...
def test_env_var_visible_to_git_provider_after_extra_merge(
    ...
    import garda.git_providers.utils as utils_mod
...
def test_resolve_accepts_windows_drive_letter_path(monkeypatch):
    ...
    monkeypatch.setattr(
        "garda.git_providers.utils.os.path.isfile",
...
def test_safe_url_for_log_strips_credentials():
    ...
    from garda.git_providers.utils import _safe_url_for_log

    raw = "https://alice:s3cret@config.example.com:8443/garda-code-action/shared.toml?private_token=xyz"
    ...
    assert safe == "https://config.example.com:8443/garda-code-action/shared.toml"
...
def test_env_var_overrides_repo_settings_when_default_exists(
    ...
    path: env vars must still win over repo-local .garda.toml for keys

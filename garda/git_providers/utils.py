...
from garda.custom_merge_loader import validate_file_security
from garda.git_providers import get_git_provider_with_context
...
def _resolve_extra_config_to_file(source):
    ...
    Accepts:
      - http:// or https:// URL: fetched via urllib (with optional auth header
        from GARDA_EXTRA_CONFIG_AUTH_HEADER, e.g. "PRIVATE-TOKEN: <token>").
    ...
    auth_header = os.environ.get("GARDA_EXTRA_CONFIG_AUTH_HEADER")
    if auth_header:
        if ":" in auth_header:
            ...
            get_logger().warning(
                "GARDA_EXTRA_CONFIG_AUTH_HEADER is set but malformed "
...
def _reapply_env_overrides():
    ...
    config file or repo .garda.toml can silently replace a secret supplied
...
def _apply_settings_from_file(path: str, label: str):
    ...
    try:
        dynconf_kwargs = {
            "core_loaders": [],
            "loaders": ["garda.custom_merge_loader"],
            "merge_enabled": True,
        }
        try:
        ...
        # Do NOT log the merged dict: external/repo .garda.toml may contain
...
def apply_repo_settings(pr_url):
    ...
    # extra file. Repo-local .garda.toml is still applied later and overrides
    ...
    try:
        ...
        for err in config_errors:
            if err:
                ...
                header = f"❌ **Garda Code Action failed to apply '{config_type}' repo settings**"
                body = f"{header}\n\nThe configuration file needs to be a valid [TOML](https://github.com/neosantara-xyz/garda-code-action/tree/main/docs/docs/usage-guide/configuration_options/), please fix it.\n\n"
                body += f"___\n\n**Error message:**\n`{err_message}`\n\n"
...
def set_claude_model():
    ...
    """
    model_claude = "anthropic/claude-3-5-sonnet-20240620"

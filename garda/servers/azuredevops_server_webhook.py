...
# The server listens for incoming webhooks from Azure DevOps Server and forwards them to the Garda Code Action.
...
from garda.agent.garda_agent import GardaAgent, command2class
...
async def _perform_commands_azure(commands_conf: str, agent: GardaAgent, api_url: str, log_context: dict):
...
async def handle_request_azure(data, log_context):
    if data["eventType"] == "git.pullrequest.created":
        ...
        await _perform_commands_azure("pr_commands", GardaAgent(), pr_url, log_context)
        return JSONResponse(

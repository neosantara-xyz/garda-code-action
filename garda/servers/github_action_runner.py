...
from garda.git_providers import get_git_provider
from garda.git_providers.utils import apply_repo_settings
from garda.log import get_logger
from garda.servers.github_app import handle_line_comments
...
async def run_action():
    ...
    NEOSANTARA_KEY = os.environ.get('NEOSANTARA_API_KEY') or os.environ.get('NEOSANTARA_KEY') or os.environ.get('NEOSANTARA.KEY')
    NEOSANTARA_BASE_URL = os.environ.get('NEOSANTARA_BASE_URL') or os.environ.get('NEOSANTARA.BASE_URL')
    AI_HANDLER = os.environ.get('GARDA_AI_HANDLER') or os.environ.get('AI_HANDLER')
    ...
    if NEOSANTARA_KEY:
        get_settings().set("NEOSANTARA.KEY", NEOSANTARA_KEY)
    else:
        # Neosantara is the default for Garda Code Action, but legacy providers may not need it.
        print("NEOSANTARA_API_KEY not set")
    if NEOSANTARA_BASE_URL:
        get_settings().set("NEOSANTARA.BASE_URL", NEOSANTARA_BASE_URL)
    if AI_HANDLER:
        get_settings().set("CONFIG.AI_HANDLER", AI_HANDLER)
    if OPENAI_KEY:
        get_settings().set("OPENAI.KEY", OPENAI_KEY)
    elif not NEOSANTARA_KEY:

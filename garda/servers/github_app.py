...
from garda.git_providers.git_provider import IncrementalPR
from garda.git_providers.utils import apply_repo_settings
from garda.identity_providers import get_identity_provider
from garda.identity_providers.identity_provider import Eligibility
from garda.log import LoggingFormat, get_logger, setup_logger
from garda.servers.utils import DefaultDictWithTimeout, verify_signature
...
def handle_closed_pr(body, event, action, log_context):
    ...
    log_context["api_url"] = api_url
    get_logger().info("Garda Code Action statistics for closed PR", analytics=True, pr_statistics=pr_statistics, **log_context)
...
def is_bot_user(sender, sender_type):
    try:
        ...
        if get_settings().get("GITHUB_APP.IGNORE_BOT_PR", False) and sender_type == "Bot":
            if 'garda-code-action' not in sender:
...
async def _perform_auto_commands_github(commands_conf: str, agent: GardaAgent, body: dict, api_url: str,
                                        log_context: dict):
    apply_repo_settings(api_url)

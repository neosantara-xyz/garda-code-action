...
from garda.algo.pr_processing import (OUTPUT_BUFFER_TOKENS_HARD_THRESHOLD,
                                         get_pr_diff,
                                         get_pr_diff_multiple_patchs,
                                         retry_with_fallback_models)
from garda.algo.token_handler import TokenHandler
from garda.algo.utils import (ModelType, PRDescriptionHeader, clip_tokens,
                                 get_max_tokens, get_user_labels, load_yaml,
                                 set_custom_labels,
                                 show_relevant_configurations)
from garda.config_loader import get_settings
from garda.git_providers import (GithubProvider, get_git_provider,
                                    get_git_provider_with_context)
from garda.git_providers.git_provider import get_main_pr_language
from garda.log import get_logger
from garda.servers.help import HelpMessage
from garda.tools.ticket_pr_compliance_check import (
...
class PRDescription:
    def __init__(self, pr_url: str, args: list = None,
                 ai_handler: partial[BaseAiHandler,] = DefaultAIHandler):
        """
            ...
            if self.git_provider.is_supported("gfm_markdown") and get_settings().pr_description.enable_help_text:
                ...
                if isinstance(self.git_provider, GithubProvider):
                    pr_body += ('\n\n___\n\n> <details> <summary>  Need help?</summary><li>Type <code>/help how to ...</code> '
                                'in the comments thread for any questions about Garda Code Action usage.</li><li>Check out the '
                                '<a href="https://github.com/neosantara-xyz/garda-code-action/tree/main/docs/docs/usage-guide/">documentation</a> '
                                'for more information.</li></details>')
                else: # gitlab
                    pr_body += ("\n\n___\n\n<details><summary>Need help?</summary>- Type <code>/help how to ...</code> in the comments "
                                "thread for any questions about Garda Code Action usage.<br>- Check out the "
                                "<a href='https://github.com/neosantara-xyz/garda-code-action/tree/main/docs/docs/usage-guide/'>documentation</a> for more information.</details>")
            # elif get_settings().pr_description.enable_help_comment:
            #     pr_body += '\n\n___\n\n> 💡 **Garda Code Action usage**: Comment `/help "your question"` on any pull request to receive relevant information'
    ...
    async def _prepare_prediction(self, model: str) -> None:
        if get_settings().pr_description.use_description_markers and 'garda:' not in self.user_description:
...
"""
    ...
    def _prepare_pr_answer_with_markers(self) -> Tuple[str, str, str, List[dict]]:
        ...
        if get_settings().pr_description.include_generated_by_header:
            ai_header = f"### 🤖 Generated by Garda Code Action at {self.git_provider.last_commit_id.sha}\n\n"
        else:
            ai_header = ""

        ai_type = self.data.get('type')
        if ai_type and not re.search(r'<!--\s*garda:type\s*-->', body):
            if isinstance(ai_type, list):
                pr_type = ', '.join(str(t) for t in ai_type)
            else:
                pr_type = ai_type
            pr_type = f"{ai_header}{pr_type}"
            body = body.replace('garda:type', pr_type)

        ai_summary = self.data.get('description')
        if ai_summary and not re.search(r'<!--\s*garda:summary\s*-->', body):
            summary = f"{ai_header}{ai_summary}"
            body = body.replace('garda:summary', summary)

        ai_walkthrough = self.data.get('pr_files')
        walkthrough_gfm = ""
        pr_file_changes = []
        if ai_walkthrough and not re.search(r'<!--\s*garda:walkthrough\s*-->', body):
            try:
                walkthrough_gfm, pr_file_changes = self.process_pr_files_prediction(walkthrough_gfm,
                                                                                    self.file_label_dict)
                body = body.replace('garda:walkthrough', walkthrough_gfm)
            except Exception as e:
                get_logger().error(f"Failing to process walkthrough {self.pr_id}: {e}")
                body = body.replace('garda:walkthrough', "")

        # Add support for garda:diagram marker (plain and HTML comment formats)
        ai_diagram = self.data.get('changes_diagram')
        if ai_diagram:
            body = re.sub(r'<!--\s*garda:diagram\s*-->|garda:diagram', ai_diagram, body)

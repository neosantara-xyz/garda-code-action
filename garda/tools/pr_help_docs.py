...
from garda.algo.utils import clip_tokens, get_max_tokens, load_yaml, ModelType
...
def extract_model_answer_and_relevant_sources(ai_response: str) -> str | None:
    ...
    # (for more details, see here: https://github.com/neosantara-xyz/garda-code-action-pro/blob/main/garda/tools/pr_help_message.py#L173)
    """
    For example:
...
class PRHelpDocs(object):
    def __init__(self, ctx_url, ai_handler:partial[BaseAiHandler,] = DefaultAIHandler, args: tuple[str]=None, return_as_string: bool=False):
        try:

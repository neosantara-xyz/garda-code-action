"""Text router: turn inbound MOSAICO text into a garda-code-action command and return the
rendered markdown.
...
from garda.mosaico.diff_provider import parse_unified_diff
...
def _empty_fallback(verb: str) -> str:
    return f"Garda Code Action {verb}: no output produced (e.g. no files/changes detected)."


def _error_fallback(verb: str) -> str:
    return f"Garda Code Action could not complete the {verb} (internal error; see agent logs)."
...
def _ask_needs_context_fallback() -> str:
    ...
    return "Garda Code Action requires a PR URL or a supplied diff."


async def _run_garda(target: str, verb: str) -> str:
    """Run a review/improve/describe verb via GardaAgent.handle_request, defensively.
    ...
    ok = await GardaAgent().handle_request(
        target,
...
async def _run_ask(target: str, question: str) -> str:
    ...
    arg-injection trick used by _run_garda cannot apply here. Instead, force
...
async def route_and_run(user_text: str) -> str:
    """Route inbound text to a garda-code-action command and return rendered markdown. Never raises."""
    try:
        text = user_text or ""
        ...
        if pr_url:
            ...
            return await _run_garda(pr_url, verb)
        ...
        if _looks_like_diff(text):
            ...
            return await _run_garda("mosaico://supplied-diff", verb)

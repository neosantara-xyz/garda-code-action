...
GardaAgentExecutor.execute runs a single non-streaming path: it FIRST installs a
...
the inbound text to a garda-code-action command, and completes the Task with the rendered
...
from garda.mosaico.dispatch import route_and_run
from garda.mosaico.observability import (langfuse_span,
                                            mosaico_log_context,
                                            parse_observability_metadata)


class GardaAgentExecutor(AgentExecutor):
    """Turns a MOSAICO message/send into a garda-code-action run and returns a text Message."""
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        raise NotImplementedError("cancel is not supported by the Garda Code Action solution agent")


async def health_check() -> str:
    """LLM-connectivity probe for /health. NO-RETRY: bypasses garda-code-action's retry-wrapped
    ...
    try:
        ...
        # Construct the handler purely for its side effect of applying garda-code-action's LLM
        # config (api_base/key/callbacks/etc.) onto the litellm module — do NOT call its
        # retry-wrapped chat_completion.
        from garda.algo.ai_handlers.litellm_ai_handler import \
            LiteLLMAIHandler

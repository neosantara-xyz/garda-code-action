...
from garda.algo.git_patch_processing import extract_hunk_lines_from_patch
from garda.algo.token_handler import TokenEncoder
from garda.algo.types import FilePatchInfo
...
class PRReviewHeader(str, Enum):
    REGULAR = "## Garda Review Guide"
    INCREMENTAL = "## Incremental Garda Review Guide"
...
def convert_to_markdown_v2(output_data: dict,
    ...
    if not incremental_review:
        ...
        markdown_text += f"⏮️ Review for commits since previous Garda Code Action review {incremental_review}.\n\n"
...
def get_max_tokens(model):
    ...
    logic:
    (1) If the model is in './garda/algo/__init__.py', use the value from there.
    ...
    if model in MAX_TOKENS:
        ...
        get_logger().error(f"Model {model} is not defined in MAX_TOKENS in ./garda/algo/__init__.py and no custom_model_max_tokens is set")
        raise Exception(f"Ensure {model} is defined in MAX_TOKENS in ./garda/algo/__init__.py or set a positive value for it in config.custom_model_max_tokens")
...
def show_relevant_configurations(relevant_section: str) -> str:
    ...
    markdown_text +="<br>These are the relevant [configurations](https://github.com/neosantara-xyz/garda-code-action/blob/main/garda/settings/configuration.toml) for this tool:\n\n"
...
def get_version() -> str:
    ...
    try:
        return version('garda-code-action')
    except PackageNotFoundError:
        get_logger().warning("Unable to find package named 'garda-code-action'")
        return "unknown"

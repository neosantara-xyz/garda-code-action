...
from garda.algo.utils import get_version
...
def set_parser():
    ...
    """)
    parser.add_argument('--version', action='version', version=f'garda-code-action {get_version()}')
    parser.add_argument('--pr_url', type=str, help='The URL of the PR to review', default=None)
    parser.add_argument('--issue_url', type=str, help='The URL of the Issue to review', default=None)
    parser.add_argument(
        "--extra_config_url",
        type=str,
        default=os.environ.get("GARDA_EXTRA_CONFIG_URL"),
        help=(
            "URL or local path of an additional .garda.toml to merge before the "
            "repo-local config (e.g. shared/org defaults). Accepts http(s):// URLs or "
            "a filesystem path. For private endpoints, set GARDA_EXTRA_CONFIG_AUTH_HEADER "
            "(e.g. 'PRIVATE-TOKEN: <token>' or 'JOB-TOKEN: $CI_JOB_TOKEN'). "
            "Repo-local .garda.toml overrides values set here."
        ),
    )
...
def run(inargs=None, args=None):
    ...
    async def inner():
        if args.issue_url:
            result = await asyncio.create_task(GardaAgent().handle_request(args.issue_url, [command] + args.rest))
        else:
            result = await asyncio.create_task(GardaAgent().handle_request(args.pr_url, [command] + args.rest))

# Garda Code Action MOSAICO Solution Agent — registration

This directory holds the MOSAICO registration template for running garda-code-action as a
MOSAICO A2A *solution agent* (in-process A2A server, image target `mosaico_agent`).

## Build & run the image

```bash
# from the garda-code-action repo root
docker build --target mosaico_agent -t garda-code-action-mosaico -f docker/Dockerfile .
docker run -d -p 9000:9000 --name garda-code-action-mosaico garda-code-action-mosaico
```
...
AGENT_CARD_URL=http://garda-code-action-mosaico:9000/.well-known/agent-card.json \
python register-agent.py
```

- `garda-code-action-solution-agent.json` is the template in this directory (kept in MOSAICO's
...
An offline-tested diagnostic (`garda/mosaico/probe.py`) that, run behind the VPN, health-checks
...
PYTHONPATH=. python -m garda.mosaico.probe

# explicit reference-agent URL + task text
PYTHONPATH=. python -m garda.mosaico.probe http://116.203.57.210:4000 "Review https://github.com/org/repo/pull/1"

# optional registry baseline (best-effort; repo may be internal-only)
MOSAICO_REPOSITORY_URL=http://116.203.57.210:8080 PYTHONPATH=. python -m garda.mosaico.probe
```
...
- the registry baseline line — `garda-code-action-solution-agent` will show **ABSENT (not yet registered)**

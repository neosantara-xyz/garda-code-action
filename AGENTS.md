...
- **Do** adjust configuration through `.garda.toml` or files under `garda/settings/` instead of hard-coding values.
...
Garda Code Action automates AI-assisted reviews for pull requests across multiple git providers.

- `garda/agent/` orchestrates commands (`review`, `describe`, `improve`, etc.) via `garda/agent/garda_agent.py`.
- `garda/tools/` implements individual capabilities such as reviewers, code suggestions, docs updates, and label generation.
- `garda/git_providers/` and `garda/identity_providers/` handle integrations with GitHub, GitLab, Bitbucket, Azure DevOps, and secrets.
- `garda/settings/` stores Dynaconf defaults (prompts, configuration templates, ignore lists) respected at runtime; `.garda.toml` overrides repository-level behavior.
...
- `.github/workflows/` defines CI pipelines for unit tests, coverage, docs deployment, pre-commit, and Garda Code Action self-review.
...
- Execute the CLI locally once dependencies and API keys are available: `python -m garda.cli --pr_url <https://host/org/repo/pull/123> review`.
...
- Configuration files in `garda/settings/` are TOML; preserve formatting, section order, and comments when editing prompts or defaults.
...
- Prefer focused unit tests that isolate helpers in `garda/algo/`, `garda/tools/`, or provider adapters; use parameterized tests where existing files already do so.
...
- Treat prompt and configuration files as single sources of truth—update mirrors (`.garda.toml`, `garda/settings/*.toml`) together when behavior changes.
...
- Adjust runtime behavior by overriding keys in `.garda.toml` or by supplying repository-specific Dynaconf files; keep overrides minimal and documented inside the PR description.
